...
public class Connection {
	/**
	 * A thread that checks every 30 seconds to see when data was last. If it's been more than two minutes, the socket is
	 * closed. This results in the read thread removing this connection from the connection map. This makes it so that if
	 * a client gets disconnected and forgets to destroy the connection (or is unable to due to network failure), the
	 * connection will still eventually be closed.
	 * 
	...
	public BlockingQueue<byte[]> receiveQueue = new LinkedBlockingQueue<byte[]>(Server.configReceiveQueueSize);
